...
__device__ float inoise_3d___(float x, float y, float z) {
	...
	float lacunarity, float gain, float freq, float amp)
{
	...
	for (int i = 0; i<octaves; i++) {
		sum += inoise_3d___(x*freq, y*freq, z*freq)*amp;
		freq *= lacunarity;
		amp *= gain;
	}
	return sum;
...
__global__ void projection(float4 *vel, cudaExtent gres, float3 invCellSize, int bx) {
	...
	if (x<gres.width && y<gres.height && z<gres.depth) {
		...
		(*pixel).x = vMask.x * vNew.x + obstV.x;
		(*pixel).y = vMask.y * vNew.y + obstV.y;
		(*pixel).z = vMask.z * vNew.z + obstV.z;
		(*pixel).w = vMask.w * vNew.w + obstV.w;
		//*pixel = vNew;
