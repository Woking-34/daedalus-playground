...
const int kParticleHeight = 16 * multiplier;
const int kParticleWidth = 8 * multiplier;
const int kParticleLength = 16 * multiplier;
...
{	
	...
	case 5:
	{
        ...
        bool fileFound = findFullPath("data/granular/", filePath);

		// voxelize mesh
        Mesh* m = ImportMeshFromPly(filePath.c_str());
		...
		if (m)
		{
            ...
            Vector3 outMinExtents, outMaxExtents;
            m->GetBounds(outMinExtents, outMaxExtents);

            Voxelize(*m, dx, dy, dz, volume, outMinExtents, outMaxExtents);
