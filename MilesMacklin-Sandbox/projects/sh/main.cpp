...
void shTests()
{
    ...
    std::string filePath = "beach.pfm";

    bool fileFound = findFullPath(std::string(SAMPLE_NAME) + rootStr, filePath);

    ProbeSampler sampler(filePath.c_str());
...
void Update(void)
{
	...
	*/
	...
	g_mesh->Transform(RotationMatrix(kPi/180*0.1f, Vec3(0.0f, 1.0f, 0.0f)));
}
...
int main(int argc, char* argv[])
{
    ...
    std::string rootStr = "/data/";
    std::string filePath = "happy.obj";

    bool fileFound = findFullPath(std::string(SAMPLE_NAME) + rootStr, filePath);

    g_mesh = ImportMeshFromObj(filePath.c_str());
