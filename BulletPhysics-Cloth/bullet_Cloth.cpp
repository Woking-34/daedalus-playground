...
void InitBulletPhysics()
{
	...
	m_softRigidDynamicsWorld		 = new btSoftRigidDynamicsWorld(m_dispatcher,m_broadphase,m_solver,m_collisionConfiguration);

	m_softBodyWorldInfo.m_dispatcher = m_dispatcher;
	m_softBodyWorldInfo.m_broadphase = m_broadphase;
	m_dynamicsWorld					 = m_softRigidDynamicsWorld;
	...
	//Creating some dynamic sphereShape
	{
		...
		for(int j=0; j<200; j++)
		{
			startTransform.setOrigin(btVector3(5,(j*5)+10,2));
	...
	//Create a cloth patch
	const btScalar	s=5;
	const btScalar	h=12;
	const int		r=18;

	btSoftBody*		psb=btSoftBodyHelpers::CreatePatch(m_softBodyWorldInfo,btVector3(-s,h,-s),btVector3(+s,h,-s),btVector3(-s,h,+s),
		btVector3(+s,h,+s),r,r,4+8,true);

	m_softRigidDynamicsWorld-> addSoftBody(psb);
}
	...
	for (  int i=0;i<softWorld->getSoftBodyArray().size();i++)
	{
		...
		for(i=0;i<psb->m_links.size();++i)
		{
...

			{
				...
				glBegin(GL_LINES);
				glVertex3f(l.m_n[0]->m_x.getX(), l.m_n[0]->m_x.getY(), l.m_n[0]->m_x.getZ());
				glVertex3f(l.m_n[1]->m_x.getX(), l.m_n[1]->m_x.getY(), l.m_n[1]->m_x.getZ());
				glEnd();
			}
		}
	}
